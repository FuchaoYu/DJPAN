...
def load_data(data_folder, batch_size, train, num_workers=0, **kwargs):
    transform = {
        'train': transforms.Compose(
            [transforms.Resize([256, 256]),
             transforms.RandomCrop(224),  # 对图像随机裁剪出尺寸为224的图片
    ...
    train_data_loader = get_data_loader(train_dataset, batch_size=batch_size,
                                        shuffle=True if train else False,
                                        num_workers=num_workers, **kwargs, drop_last=True if train else False)

    test_data_loader = get_data_loader(test_dataset, batch_size=batch_size,
                                       shuffle=False,
                                       infinite_data_loader=False,
                                       num_workers=num_workers, drop_last=False)

    n_class = len(data.classes)
    return data_loader, train_data_loader, test_data_loader, n_class
...
class InfiniteDataLoader:
    def __init__(self, dataset, batch_size, shuffle=True, drop_last=False, num_workers=0, weights=None, **kwargs):
        if weights is not None:  # 设置不同样本采样权重
            sampler = torch.utils.data.WeightedRandomSampler(weights,
                                                             replacement=False,
                                                             num_samples=batch_size)
        else:
            sampler = torch.utils.data.RandomSampler(dataset,
                                                     replacement=False)  # 对数据随机采样
