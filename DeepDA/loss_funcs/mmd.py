...
class MMDLoss(nn.Module):
    ...
    def guassian_kernel(self, source, target, kernel_mul, kernel_num, fix_sigma):
        ...
        kernel_val = [torch.exp(-L2_distance / bandwidth_temp)  # 前面根据mul与num得到了5个bandwidth值，此时便有5个kernel。或者理解为σ取多个值，分别求核函数然后取和，作为最后的核函数。
    ...
    def forward(self, source, target):
        if self.kernel_type == 'linear':
            ...
            XX = torch.mean(kernels[:batch_size, :batch_size]) # 求平均值的过程即为1/ns
